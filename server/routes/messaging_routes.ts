...
const messagesStore: ProjectMessage[] = [
  {
    ...
    sender: 'admin',
    senderName: 'Jason Moyer (Axiom OS Lead)',
    senderEmail: 'jason@moyervllc.com',
    text: 'Welcome to Axiom OS! Your DocuFlow AI venture is currently at Gate 4 (Stripe checkout verification). Let me know if you need any adjustments or custom domain setup.',
...
messagingRoutes.post('/:ventureId', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const newMessage: ProjectMessage = {
      ...
      senderName: senderName || (sender === 'admin' ? 'Jason Moyer (Axiom OS)' : 'Founder'),
